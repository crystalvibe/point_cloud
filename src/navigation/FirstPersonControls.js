...
export class FirstPersonControls extends EventDispatcher {
	...
	update (delta) {
		...
		if (this.scene.scenePointCloud.children.length === 0) {
			return;
		}

		// Use initial move speed if available, otherwise use current speed
		let moveSpeed = this.viewer.initialMoveSpeed || this.viewer.getMoveSpeed();

		if (this.keys.FORWARD.some(e => this.viewer.inputHandler.pressedKeys[e])) {
			view.position.add(view.direction.clone().multiplyScalar(moveSpeed * delta));
		}
		if (this.keys.BACKWARD.some(e => this.viewer.inputHandler.pressedKeys[e])) {
			view.position.add(view.direction.clone().multiplyScalar(-moveSpeed * delta));
		}
		if (this.keys.LEFT.some(e => this.viewer.inputHandler.pressedKeys[e])) {
			let right = view.direction.clone().cross(new THREE.Vector3(0, 0, 1));
			view.position.add(right.multiplyScalar(-moveSpeed * delta));
		}
		if (this.keys.RIGHT.some(e => this.viewer.inputHandler.pressedKeys[e])) {
			let right = view.direction.clone().cross(new THREE.Vector3(0, 0, 1));
			view.position.add(right.multiplyScalar(moveSpeed * delta));
		}

		view.pitch = view.pitch + this.pitchDelta * delta;
		view.yaw = view.yaw + this.yawDelta * delta;
