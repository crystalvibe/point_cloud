...
export class Viewer extends EventDispatcher{
	...
	constructor(domElement, args = {}){
...

		{
			...
			// Force orbit controls immediately
			this.setControls(this.orbitControls);
			if (this.orbitControls) {
				this.orbitControls.enabled = true;
				this.orbitControls.initializeControls();
			}

			// Completely disable earth controls
			if (this.earthControls) {
				this.earthControls.enabled = false;
				this.earthControls.disable();
			}
			...
			this.addEventListener('scene_changed', (e) => {
				...
				if(!e.scene.hasEventListener("pointcloud_added", this.onPointCloudAdded)){
					e.scene.addEventListener("pointcloud_added", this.onPointCloudAdded);
				}
				...
				// Force orbit controls after scene change
				this.setControls(this.orbitControls);
				if (this.orbitControls) {
					this.orbitControls.enabled = true;
					this.orbitControls.initializeControls();
				...
				// Ensure earth controls are disabled
				if (this.earthControls) {
					this.earthControls.enabled = false;
					this.earthControls.disable();
				}
		...
		// Final check to ensure orbit controls
		setTimeout(() => {
			this.setControls(this.orbitControls);
			if (this.orbitControls) {
				this.orbitControls.enabled = true;
	...
	setScene (scene) {
		if (this.scene) {
			this.scene.remove(this.scene.root);
		}
		...
		{ // Annotation
			if(oldScene){
				oldScene.annotations.traverse(annotation => {
					annotation.display.visible = false;
				});
			}

			if(scene){
				this.scene.annotations.traverse(annotation => {
					annotation.display.visible = annotation.visible;
				});
			}
		}
		...
		{ // Background
			if(oldScene){
				oldScene.background.traverse(node => {
					node.visible = false;
				});
			}
			
			if(scene){
				scene.background.traverse(node => {
					node.visible = node.visible;
				});
			}
		}
		
		// Special handling for camera mode changes
		if (scene && scene.view) {
			// Always set orbit control mode for the scene
			if (this.orbitControls) {
				this.orbitControls.enabled = true;
				this.setControls(this.orbitControls);
		...
		// Force orbit control mode for scene
		if (this.orbitControls && this.orbitControls.setScene) {
			try {
				this.orbitControls.setScene(scene);
			} catch (e) {
				console.warn('Error setting scene for orbit controls:', e);
			}
		}

		{
			...
			if(!this.scene.view){
				this.scene.view = new Potree.View(camera.clone());
			}

			// Initialize camera position and orientation for Orbit Controls
			let box = this.getBoundingBox(scene.pointclouds);
			...
			camera.rotation.x = Math.PI / 4;  // 45 degree angle
			camera.rotation.z = 0;

			// Update view
			this.scene.view.position.copy(camera.position);
			this.scene.view.radius = maxDim * 2;
			this.scene.view.pitch = -Math.PI / 4;  // 45 degree angle
			this.scene.view.yaw = 0;

			// Set orbit controls as default
			this.setControls(this.orbitControls);
			this.orbitControls.enabled = true;
		}
	}
	...
	fitToScreen (factor = 1, animationDuration = 0) {
		// Only execute if explicitly called by user action
		if (this.scene.pointclouds.length === 0 || this.controls.isLoading) {
			return;
		}
		...
		// Calculate height for bird's eye view
		let size = box.getSize(new THREE.Vector3());
		let maxDim = Math.max(size.x, size.y, size.z);
		
		// Set camera position for bird's eye view
		this.scene.view.position.set(0, 0, maxDim * 2);
	...
	createControls() {
		// Create orbit controls first and set as default
		this.orbitControls = new OrbitControls(this);
		this.orbitControls.enabled = true;
		this.orbitControls.initializeControls();

		// Create other controls but keep them disabled
		...
		// Force orbit controls as default
		this.controls = this.orbitControls;
		this.setControls(this.orbitControls);
	}
	...
	update(delta, timestamp){
		// Skip updates while scene is loading/transitioning
		if (this.isLoading) return;

		// Always ensure orbit controls are active
		if (this.controls !== this.orbitControls && this.orbitControls) {
			this.setControls(this.orbitControls);
		}
		...
		// Update camera with unrestricted rotation
		const scene = this.scene;
		const view = scene.view;

		// Update perspective camera without any rotation limits
		scene.cameraP.position.copy(view.position);
		scene.cameraP.rotation.order = "ZXY";
		scene.cameraP.rotation.x = view.pitch;
		scene.cameraP.rotation.z = view.yaw;

		// Update orthographic camera without any rotation limits
		scene.cameraO.position.copy(view.position);
		scene.cameraO.rotation.order = "ZXY";
		scene.cameraO.rotation.x = view.pitch;
		scene.cameraO.rotation.z = view.yaw;

		// Normal updates continue below
		if(this.getMaterialQuality() === 'timedBlend'){
			if(this.timedBlendContainer.children.length === 0){
				// ...
			}
		}
	...
	onPointCloudAdded(e){
		// Always set orbit controls as default
		this.setControls(this.orbitControls);
		...
		if(this.scene.pointclouds.length === 1){
			// Calculate initial position based on bounding box
			let box = e.pointcloud.boundingBox;
			let center = box.getCenter(new THREE.Vector3());
			let size = box.getSize(new THREE.Vector3());
			let maxDim = Math.max(size.x, size.y, size.z);
			
			// Set initial camera position for unrestricted viewing
			...
			// Initialize view with no rotation
			this.scene.view.position.copy(position);
			this.scene.view.lookAt(center);
			this.scene.view.radius = maxDim * 2;
			this.scene.view.pitch = 0;
			this.scene.view.yaw = 0;
			
			// Set move speed based on point cloud size
			let speed = maxDim;
			this.setMoveSpeed(speed);
			this.initialMoveSpeed = speed;
		}
		
		// Final check to ensure orbit mode
		setTimeout(() => {
			this.setControls(this.orbitControls);
			if (this.orbitControls) {
				this.orbitControls.enabled = true;
